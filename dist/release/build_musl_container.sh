#!/usr/bin/env sh
# Builds a static musl fusibile binary. Runs INSIDE the Alpine container
...
cargo build --locked --release --target "$TARGET" -p fusibile --features smb-vendored

# -- prove the binary is static: no interpreter, no shared libraries
file "target/$TARGET/release/fusibile"
if readelf -l "target/$TARGET/release/fusibile" > /tmp/program-headers.txt; then
...
if readelf -d "target/$TARGET/release/fusibile" > /tmp/dynamic-section.txt; then
